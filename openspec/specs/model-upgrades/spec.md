...
Defines the workshop's upgrade bench — the counter where capacity for a model is bought,
laid out by the family and the knob each price moves — so that *this dial does not go past
two yet, and here is what going past two costs* is a sentence a student can read in one
place, and so that a capacity item is never sold where the knob it opens cannot be seen.
