...
## 6. The inversion guard, the recorded ladder position, and the claim
...
- [x] 6.2 Add the inversion guard: no declared feature's separation over the evaluation split exceeds its separation over the fitted images beyond the declared tolerance, refusing with the feature and both figures, and verify it against the real pool
...
- [x] 6.3a Reconcile the inversion requirement's normative sentence and its first scenario with its rationale, its second scenario and `checkNoInversion` — the two halves currently forbid opposite directions — and verify a test names the direction being forbidden rather than only exercising it
- [x] 6.4 Implement the bounded brute-force rule search over the declared features within the declared node budget, thresholds taken as midpoints between adjacent observed values subsampled to a declared cap, and verify it runs over the manifest without rasterizing and completes inside the normal test run
- [x] 6.5 Replace the ladder refusal with a recording: turn `ladderIssues` into a comparison that reports the measured figures rather than refusing on their direction, drop `weakestModel`'s role as a floor, and verify the module's own documentation no longer describes a refusal
- [x] 6.6 Record the ladder position against every shipped configuration in both currencies — category scores over the evaluation split, and earnings over a year's crop at the mildest and wettest declared composition, reusing the crop and delivery machinery `test/delivery-guards.test.ts` already assembles — and verify each recorded figure is pinned so a change that moves it fails naming the figure, its recorded value and its measured value
- [x] 6.7 Verify the recording is taken at the largest node budget the task declares, and that raising the declared budget fails until the figures are re-recorded
- [x] 6.8 Add the claim check over the declared teaching copy, the declared market copy and the screens, and verify it refuses a fixture whose copy claims an advantage the recorded figures measure the other way
...
- [x] 9.1 Run the full suite and `npm run typecheck`, and verify both pass
...
- [x] 9.4 Verify no remaining wording in the delta specs, `src/features/` or the tests calls the evaluation split "the harvest", now that `harvest-run` uses that word for a year's crop
