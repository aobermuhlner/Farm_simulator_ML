...
**Reason**: The word "family" now names two different things. A ladder rung a student buys
is a *model family* (`model-families`); a choice of how knobs are interpreted and drawn —
fully-connected or convolutional — is an *architecture kind*, which is already the word
`task-contract` uses for it in *A task may declare how its architecture is drawn* and the
word the code uses in its diagram kinds. Keeping both under one noun would make every
requirement mentioning a family ambiguous. Replaced by *A model family declares an
architecture kind*, which also moves the declaration from the task to the family, since a
task now declares several and they do not share an architecture.

**Migration**: A task that declared one architecture declares one family that declares that
architecture kind. No architecture kind is removed and no knob changes meaning.

### Requirement: Capacity knobs name blocks and channels in the convolutional family

**Reason**: Renamed for the same collision, and rescoped: the knobs belong to a family
rather than to a task. Replaced by *Capacity knobs name blocks and channels in the
convolutional kind*, whose content is otherwise unchanged.

**Migration**: None. The same knobs select the same things for the family that declares
them.

### Requirement: Each architecture family is usable independently of any one screen

**Reason**: Renamed for the same collision. Replaced by *Each architecture kind is usable
independently of any one screen*, whose content is unchanged apart from the noun.
