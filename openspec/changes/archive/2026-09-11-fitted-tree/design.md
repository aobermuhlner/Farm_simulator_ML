...
See `proposal.md` — Why, for what this rung carries and the scope it is held to.

**A note on what this document is now.** It was written for a change that fitted a tree
offline and measured what the fit exhibited. That depth is deferred (`proposal.md` —
Deferred) and the decisions recording it are kept rather than deleted: they were expensive
to reach, they are still the right answers, and the change that lands the fitting should
honour them rather than re-derive them. Where a decision below describes fitting, read it as
binding on that later change, not on this one. Where it describes what ships and how the
browser reads it, it binds now.
...
- The frame carries a second family end to end: bought, gated by a puzzle, selected, tuned,
  drawn, fielded and reported, from declared data alone.
- The tree that scores the harvest and the tree drawn on screen are one object, so they
  cannot drift apart.
- The stranded `first-tree-leaves` puzzle becomes reachable, which is the first real use of
  `model-tutorials`.
- The three shipped convolutional artifacts resolve byte-identically. This is a test.
- Replacing a placeholder tree with a fitted one is a data change, not a change to this one.

**Non-Goals:**

- **Fitting anything, anywhere.** The trees are authored. Per `CLAUDE.md`, nothing here
  measures how good a tree is, and no task below group 6 may. The decisions this document
  records about fitting stand as decisions for the change that lands it.
- A growth history. There is no growth. The family declares no history and the workshop
  shows none, which `model-families` already provides for.
- Fitting in the browser. Settled below, with the conditions that would reopen it.
...
### The trees ship authored, and the artifact cannot hide it

One tree per covered budget, written by hand into the shape a fit would later emit, and
recorded in its own provenance as authored. Everything downstream — the reader, the
evaluator, the drawing, the scoring, the report — sees exactly what it will see when the
trees are real, which is the whole reason to prefer a placeholder over a stub that returns a
constant distribution: a constant would exercise the plumbing without exercising the shape.

The provenance flag is not bookkeeping. Two things read it. The refusal in the spec — a tree
recording that it was fitted without recording what fitted it — depends on the distinction
being written down rather than inferred from an absent history, and inference is exactly how
a placeholder quietly becomes load-bearing. And the honesty condition on copy needs something
to check against.

*Alternative considered — ship nothing and let the family resolve to a refusal until the
fitting lands.* Rejected: the frame is what this change exists to test, and a family that
cannot be put to work tests none of the parts after selection — no harvest, no report, no
labour slot, which is where a second family is most likely to break something.

*The risk taken:* a placeholder that scores badly makes the tree look like a bad model, and
a student meeting it would learn something false. That is acceptable only while nobody is
being taught from this build, which is the standing condition of `CLAUDE.md`'s prototype
rule. It is the first thing the fitting change should retire.
...
Capacity is a node budget sold through the catalog, counted in internal nodes. (It was to
have been rung 1's budget, sold for a tree the student wrote; that rung is withdrawn — see
`proposal.md` — Why — and the budget is declared by this family instead. Nothing else about
the decision moves.) The fit grows the tree **best-first** — at each step taking the split that
...
Rejected: depth is a second capacity currency next to a node budget, it is the harder of the
two to price and to explain, and depth-limited growth is breadth-nested rather than
...
### Leaves are distributions; the reference tree is declared data
...
must stay a live computation. It is also the teaching line: the tree reports how sure it is
and the policy decides what to do about it, which is the same division the network is under.

*Deferred, and kept as a decision.* The reference tree the eventual floor is measured against
is declared alongside the family, seeded from `Game_design.md` §5.4's mockup. Declaring it
rather than hard-coding it keeps the floor auditable and keeps the pipeline from naming a
feature. Nothing measures against it in this change — there is nothing yet to measure.
