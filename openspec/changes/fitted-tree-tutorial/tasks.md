...
**Spec sync so far, 2026-09-09.** The `model-tutorials` delta has been merged into
`openspec/specs/model-tutorials/spec.md`: those three amendments are shipped and reachable, so
they belong in the main specs now. The `fitted-tree-tutorial` delta is deliberately *not*
synced — no declaration carries the tutorial, so the capability would be recorded as built
while a student cannot open it. Sync it when this group is finished, and archive then.
