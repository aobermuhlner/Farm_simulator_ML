...
An owned id the catalog no longer declares, or a knob value a task no longer permits,
SHALL be dropped from the restored farm while the rest of the save is kept. The year, the
balance and the ledger SHALL survive it. A dropped owned id SHALL open nothing, and a
dropped knob value SHALL fall back to that knob's declared default.
