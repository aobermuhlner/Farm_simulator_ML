...
function selectable(owned: readonly string[]): string[] {
  ...
  return rows
    .filter((values) => lockedIssues({ taskId: apple.id, values }, availability).length === 0)
    .map((values) => configurationId({ taskId: apple.id, values }))
}

describe('the three refusals are distinct', () => {
  it('refuses a locked but declared value as locked, naming what opens it', () => {
    const refused = resolveSelectable(apple, { channels: 8 }, availabilityFor([]))
  ...
  it('refuses a value the declaration does not permit as invalid, whatever is owned', () => {
    for (const owned of [[], ['wider-blocks']]) {
      const refused = resolveSelectable(apple, { channels: 64 }, availabilityFor(owned))
      expect(refused.ok).toBe(false)
  ...
  it('asks invalid before locked, so an out-of-range value never reads as unowned', () => {
    const refused = resolveSelectable(apple, { channels: 64, blocks: 3 }, availabilityFor([]))
    expect(refused.ok).toBe(false)
    if (refused.ok) return
  ...
  it('lets an available but uncovered configuration through, to refuse as untrained', () => {
    ...
    const selectable = resolveSelectable(apple, { regularization: 3 }, availabilityFor([]))
  ...
  it('locks nothing when no availability is supplied', () => {
    expect(resolveSelectable(apple, { channels: 8 }, undefined).ok).toBe(true)
  })
})
...
describe('unlocking extends what can be selected and reinterprets nothing', () => {
  ...
  it('carries every knob in the identifier, each locked one at its declared default', () => {
    const opened = resolveSelectable(apple, {}, availabilityFor([]))
    expect(opened.ok).toBe(true)
    if (!opened.ok) return

    expect(opened.configuration.values.map(([id]) => id)).toEqual(apple.knobs.map((knob) => knob.id))
    expect(configurationId(opened.configuration)).toBe('blocks2-channels16-regularization1-dropout0')
  })
  ...
  it('leaves what an identifier means exactly where it was', () => {
    ...
    const owned = resolveConfiguration(apple, values)
    expect(owned.ok).toBe(true)
    if (!owned.ok) return

    for (const ownership of [[], ['wider-blocks'], ['wider-blocks', 'deeper-blocks']]) {
      const again = resolveSelectable(apple, values, availabilityFor(ownership))
  ...
  it('leaves `resolveConfiguration` unaware of ownership', () => {
    ...
    expect(resolveConfiguration(apple, { channels: 8 })).toEqual(
      resolveConfiguration(apple, { channels: 8 }),
    )
    expect(resolveConfiguration(apple, { channels: 8 }).ok).toBe(true)
  })
})
